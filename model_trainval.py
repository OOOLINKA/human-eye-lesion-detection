...
def autoencoder_train(model_id):   #训练并保存模型
	...
	model_save_path = Encoder_Model_Path + model_id + '.h'
...
def svm_train(model_id):
	...
	model_save_path = Encoder_Model_Path + model_id + '.pkl'
	joblib.dump(clf, model_save_path, compress=1)

def model_evaluate(model_name): # for all data
	data_ids = gen_model_ids(root='/Users/yangmin/zhangjing/human-eye-lesion-detection/data/')
	ORs = []
