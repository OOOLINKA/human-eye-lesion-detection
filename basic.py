...
def load_data(model_id):  # 加载训练数据
    ...
    label_mat = 'GT'
    # feats_path = data_path + 'A/' + 'A_' + model_id + '.mat'  # features
    # label_path = data_path + 'GT/' + 'GT_' + model_id + '.mat'  # label
    feats_path = root + '/A_' + model_id + '.mat'
    label_path = root + '/GT_' + model_id + '.mat'

    feats = sio.loadmat(feats_path)[feat_mat].T  # 将列特征，列标签转化为行的形式
    # feats = feats.reshape(200,200,200)
    # feats = np.mean(feats,axis=0).T
    print (feats.shape)

    label = sio.loadmat(label_path)[label_mat].T
    # label = label.reshape(200,200).T
    print(label.shape)
...
def load_group_alphas():  #加载分组权重系数
    dump_path = 'group_alphas.pkl'
    if os.path.exists(dump_path):
        print(dump_path + ' exist')
        with open(dump_path, 'rb') as f:
            group_alphas = pickle.load(f)
    else:
        print(dump_path + ' not exist')
        group_alphas = None
    return group_alphas


def rand_group6x9(Encoder=True):  # 随机生成模型分组
    ...
    if os.path.exists(dump_path):
        ...
        for root, dirs, model_names in os.walk(Encoder_Model_Path if Encoder else Svm_Model_Path):
            pass
        for i in range(6):
